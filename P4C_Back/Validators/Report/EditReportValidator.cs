...
{
    public class EditReportValidator : AbstractValidator<EditReportRequest>
    {
        private readonly AppDbContext _appDbContext;

        public EditReportValidator(AppDbContext appDbContext)
        {
            ...
            RuleFor(x => x.NomeReport).NotEmpty().MaximumLength(50).Must((x, nomeReport) => IsNomeReportUniqueEdit(nomeReport, x.Id));
        ...
        private bool IsNomeReportUniqueEdit(string nomeReport, int id)
        {
            string? reportName = _appDbContext.Reports.Where(r => r.NomeReport == nomeReport && r.Id != id).Select(r => r.NomeReport).FirstOrDefault();
